...
class ShangHaiShiJinShanQuRenMinFaYuan(KtggIterPageBase):
    domain = 'js.hshfy.sh.cn'
    ename = 'shang_hai_shi_jin_shan_qu_ren_min_fa_yuan'
    cname = u'上海市金山区人民法院'
    developer = u'何振'
    header = {'Accept': 'application/json,text/javascript,*/*;q=0.01,charset=utf-8',
              ...
              "referer": "http://www.js.hshfy.sh.cn:88/jsfy/apps/pages/index/show.php",
              'User-Agent': 'Mozilla/5.0(WindowsNT10.0;Win64;x64)AppleWebKit/537.36('
                            'KHTML,likeGecko)Chrome/69.0.3497.100Safari/537.36',
              "X-Requested-With": "XMLHttpRequest"}

    url_start = "http://www.js.hshfy.sh.cn:88/jsfy/apps/global/article/articleDetail.php?articleKey=f7980703afa5b22849bbae6d493ae93e"
    ...
    def __init__(self, logger, seed_dict):
        ...
        super(ShangHaiShiJinShanQuRenMinFaYuan, self).__init__(self.seed_dict, self.logger)

    def parse_per_page(self, html, href, raw_id, page=0):
        try:
            self.do_time = self.ktgg_tool.now_date
            soup = bs(html, 'lxml')
            bodys = soup.find("pre", {"id": "articleContent"}).find_all("p")[:-1]
            for index, line in enumerate(bodys):
                body = line.text
                data_dict = {
                    "cname": self.cname,
                    "ename": self.ename,
...
if __name__ == '__main__':
    ins = ShangHaiShiJinShanQuRenMinFaYuan(None, {"is_increment": False, "page": 1})
    a = ins.start()
    print a
