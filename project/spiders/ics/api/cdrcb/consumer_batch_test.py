...
def callback(ch, method, properties, body):  # 定义一个回调函数，用来接收生产者发送的消息
    try:
        ...
        app.send_task('ics.task.' + task['task_name'] + '.batch_test.task.start', [task],
                      queue='queue_batch_test_normal_' + task['task_name'] + default_settings.CDRCB_PROJECT_SUFFIX.lower(),
                      priority=0)
...
chan.basic_consume(
    ...
    queue=default_settings.BATCH_TEST_NORMAL_PUSH_QUEUE  # 指定取消息的队列名
    # no_ack=True
