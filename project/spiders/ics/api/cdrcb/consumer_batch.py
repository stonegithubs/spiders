...
chan.queue_declare(queue=default_settings.CDRCB_BATCH_PUSH_QUEUE,
...
def callback(ch, method, properties, body):  # 定义一个回调函数，用来接收生产者发送的消息
    try:
        ...
        app.send_task('ics.task.' + task['task_name'] + '.cdrcb.task.start', [task],
                      queue='queue_cdrcb_batch_' + task['task_name'] + default_settings.CDRCB_PROJECT_SUFFIX.lower(),
                      priority=0)
...
chan.basic_consume(
    ...
    queue=default_settings.CDRCB_BATCH_PUSH_QUEUE  # 指定取消息的队列名
    # no_ack=True
