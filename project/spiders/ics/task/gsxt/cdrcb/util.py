...
def judge_html_is_none(resp, meta):
    """
    判断页面为空
    :param resp:
    :return:
...
def judge_status_eq_521(resp, meta):
    """
    处理网页出现521加速乐的情况
    :param resp:
    :return:
...
def judge_status_gt_400(resp, meta):
...
def judge_invalid_link(resp, meta):
    html = resp.content
...
def assert_page_not_found(resp, meta):
    html = resp.content
...
def assert_is_not_json(resp, meta):
    try:
        html = resp.content
        json.loads(html)
...
def assert_invalid_cache_key_0_0(resp, meta):
    try:
        if assert_is_not_json(resp, meta) is None:
...
def assert_dos_attack(resp, meta):
    try:
        html = resp.content
...
#
#         if resp.status_code >= 400:
#             err_msg = u'下载返回状态码不合法, 状态码为： {}, 换代理重试'.format(str(resp.status_code))
#             logger.warning(err_msg)
#             downloader.proxy.abandon_proxy()
#             downloader.proxy.change_proxy()
...
#             logger.warning(u'invalid_link，链接不合法，切换代理重试')
#             downloader.proxy.abandon_proxy()
#             downloader.proxy.change_proxy()
...
#
#         if is_json is True:
#             if assert_is_json(res_html) is False:
#                 logger.warning(u'返回结果期望是json，但page非json')
#                 raise Exception(u'返回结果期望是json，但page非json， html: {}'.format(res_html))
#
#             if cache_key_switch is True and json.loads(res_html).get('cacheKey') == invalid_cache_key:
#                 logger.warning(u'执行断言函数检查失败，包含cache_key: {}， html为：{}'.format(invalid_cache_key, res_html))
#                 get_pictures()
#                 raise Exception(u'检查内容失败')
#         if u'您的访问行为被判定为DoS攻击' in res_html:
#             err_msg = u'您的访问行为被判定为DoS攻击，切换代理重试'
#             logger.warning(err_msg)
#             downloader.proxy.abandon_proxy()
#             downloader.proxy.change_proxy()
#             raise Exception(err_msg)
#         return resp
#     except Exception as e:
#         logger.error(u'下载异常, method: {} url: {}, 原因: {}'.format(method, url, str(e)))
#         time.sleep(2)
#         raise LogicException(u"下载失败")
