...
from ics.task.gsxt.batch_test.util import *
...
def get_validate():
    try:
        yzm_url = 'http://www.gsxt.gov.cn/SearchItemCaptcha?t={}'.format(current_datetime())
        params = {
            'method': 'get',
            'url': yzm_url,
        }
...
#     :return:
#     """
#     sfxz_html_list = page_dict['sfxz']
#     value_dict['sfxzxq_id_list'] = []
#     for item in sfxz_html_list:
#         try:
#             html = item.get('html', '{}')
...
#                 try:
#                     if row.get('type') == '1': #and row.get('isFreeze') == '1':
#                         url = '/corp-query-entprise-info-judiciaryStockfreeze-{}.html'.format(row['parent_Id'])
#                         sfxzxq_uri_list.append(url)
#                     elif row.get('type') != '1' and row.get('isShareHolderAlt') == '1':
#                         if value_dict['uri_dict'].get('sfxz_xq_uri'):
#                             url = value_dict['uri_dict'].get('sfxz_xq_uri')
#                             if url not in sfxzxq_uri_list:
#                                 sfxzxq_uri_list.append(url)
#                 except Exception as e:
#                     logger.warning(u'获取模块详情id列表错误 {}， row: {}'.format(str(e), row))
#             value_dict['sfxzxq_id_list'].extend(sfxzxq_uri_list)
#         except Exception as e:
#             logger.warning(u'获取模块详情id列表错误 {}'.format(str(e)))