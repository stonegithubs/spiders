    ...
    if not cnt:
        ...
        if pages > 1:
            for page in range(2, pages + 1):
                ...
                app.send_task('ics.task.fysx.batch_test.task.process_page', [seed_dic, params, page_cnt, cookie, url, cnt],
                              queue=setting.BATCH_TEST_SHIXIN_NORMAL_TASK_QUEUE, proiority=2)
        id_lst = re.compile('\"View\"\s*id=\"(\w+)\"').findall(req)
        for each_id in id_lst:
            app.send_task('ics.task.fysx.batch_test.task.process_detail', [seed_dic, each_id, cookie, cnt],
                          queue=setting.BATCH_TEST_SHIXIN_NORMAL_TASK_QUEUE,
                          proiority=3)
