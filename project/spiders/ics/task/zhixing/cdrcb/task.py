...
from ics.task.zhixing.cdrcb.etl_zhixing import zhixing_parse
...
from ics.settings.default_settings import ZHIXING_DATA_TABLE, ZHIXING_NORMAL_TASK_QUEUE
...
def start(self, seed_dict):
    ...
    for page in range(1, total_page + 1):
        app.send_task('ics.task.zhixing.cdrcb.task.visit_page_list', [seed_dict, page, total_cnt],
                      queue=ZHIXING_NORMAL_TASK_QUEUE, priority=2)
...
def visit_page_list(self, seed_dict, page, total_cnt):
    ...
    for detail_id in detail_id_list:
        app.send_task('ics.task.zhixing.cdrcb.task.visit_detail_list',
                      [seed_dict, detail_id, total_cnt],
                      queue=ZHIXING_NORMAL_TASK_QUEUE,
                      priority=3)
...
def download_page(self, method, url, headers, timeout=40, proxies={}, data=None):
    proxy = get_proxy_from_zm(database_proxy_num=5, api_num=6, black_type='zhixing', from_type='25mintest')
