...
def request_html(company_names):
    url = 'https://www.tianyancha.com/search?key={}'.format(company_names)
    headers = {
    ...
    ip = requests.get(
        'http://webapi.http.zhimacangku.com/getip?num=1&type=1&pro=&city=0&yys=0&port=1&pack=37176&ts=0&ys=0&cs=0&lb=1&sb=0&pb=4&mr=1&regions=')
    ip = ip.text
    ip = {
        'http':'http://'+ip,
    }

    resp = requests.get(url=url, headers=headers,proxies=ip, cookies=cookie).text
    with open('res.html','w',encoding='utf-8')as f:
        f.write(resp)
    company_href = re.findall(r"""tyc-event-click tyc-event-ch="CompanySearch.Company"
             href="(.*?)" target='_blank'
          ><em>""",resp)
    print(company_href[0])
    resp_detail = requests.get(url=company_href[0], headers=headers, cookies=cookie).text
