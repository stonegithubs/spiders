...
class QuNaSpider(object):
    def __init__(self):
        ...
        with open('start_date.txt', 'w', encoding='utf8')as date:
            date.write(str(datetime.datetime.now()))
...
60.166.151.111:4241"""
    ...
    def run(self, start_station, end_station):
        url = 'https://train.qunar.com/dict/open/s2s.do?callback=jQuery172012216425254778174_' + self._m + '&dptStation=' + start_station + '&arrStation=' + end_station + '&date=2019-08-30&type=normal&user=neibu&source=site&start=1&num=500&sort=3&_=' + self._m
        time.sleep(0.5)
            ...
            if res.get('s2sBeanList'):
                ...
                for j in train_list:
                    ...
                    try:
                        ...
                        two_t = ''
                    try:
                        there_p = j['seats']['一等座']['price']
                        there_t = j['seats']['一等座']['count']
                    except Exception:
                        there_p = ''
                        there_t = ''
                    try:
                    ...
                    self.connection(trainNo, dptStationName, dptTime, arrStationName, dayDifference + arrTime,
                                    str(all_station), dptDate, one_p, one_t, two_p, two_t, there_p, there_t, four_p,
                                    four_t,
    ...
    def connection(self, *args):
        ...
        cursor = conn.cursor()
        # 去重
        sqlExit = "SELECT trainNo,dptStationName,arrStationName FROM table_all_train  WHERE trainNo = '%s' and dptStationName= '%s' and arrStationName= '%s'" % (
        args[0], args[1], args[3])
        res = cursor.execute(sqlExit)  # res=重复次数
